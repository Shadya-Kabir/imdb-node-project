...
app.get('/search/:searchResult', (req, res) => {
    ...
    let findWords = (req.params.searchResult).split(" ");
    for(let j=0; j< findWords.length; j++){
      for(let i =0; i< findArray.length; i++){
        if(((findArray[i].title).toLowerCase()).includes(findWords[j])){
          findMovie.push(findArray[i]);
        }
        if(((findArray[i].year).toLowerCase()).includes(findWords[j])){
          findMovie.push(findArray[i]);
        }
      }
    } 
